...
public class LimeTorrentSearchHandler
    ...
    // break; // OK
    // case 403:
    // LOG.error("Access to LimeTorrent is forbidden. Check if the site is blocked
    // in your region.");
    // return;
    // default:
    // LOG.error("Failed to access LimeTorrent. HTTP status: {}",
    // response.status());
    // return;
    // }
    ...
    // List<ElementHandle> listItems = page.querySelectorAll("table.table2 >
    // tbody.torsearch > tr");

    // for (ElementHandle item : listItems) {
    // try (Page magPage = browser.newPage();) {
    // String title = item.querySelector("td.tdleft > div.tt-name >
    // a[class=openPopup]").innerText();
    // String sourceUrl = LIME_TORRENT_BASE_URL
    // + item.querySelector("td.tdleft > div.tt-name >
    // a[class=openPopup]").getAttribute("href");
    ...
    // URI pageUrl = URI.create(item.querySelector("td.tdleft > div.tt-name >
    // a[class=openPopup]")
    // .getAttribute("href"));
    // UrlBuilder urlBuilder =
    // UrlBuilder.fromString(LIME_TORRENT_STORE_URL).withPath(pageUrl.getPath());
    ...
    // String magnetLink =
    // magPage.querySelector("a[href*=magnet]").getAttribute("href");

    // emitter.send(
    // new SearchResponse(title, magnetLink, "LimeTorrent", sourceUrl, size,
    // seeders, leechers));
    // }
    // }
    // }
