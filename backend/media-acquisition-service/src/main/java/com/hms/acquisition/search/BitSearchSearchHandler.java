...
public class BitSearchSearchHandler implements SseEmitterHandler<SearchRequest> {
    ...
    // break; // OK
    // case 403:
    // LOG.error("Access to BitSearch is forbidden. Check if the site is blocked in
    // your region.");
    // return;
    // default:
    ...
    // List<ElementHandle> listItems = page.querySelectorAll("main.max-w-7xl >
    // div.space-y-4 > div");

    // for (ElementHandle item : listItems) {
    // String title = item.querySelector("div > div > div > h3 > a").innerText();
    // String sourceUrl = BIT_SEARCH_BASE_URL + item.querySelector("div > div > div
    // > h3 > a").getAttribute("href");
    // String magnetLink = item.querySelector("div > div >
    // a[href*=magnet]").getAttribute("href");
    // String size = item.querySelector("div > div > div > span > i.fa-download +
    // span").innerText();
    // String seeders = item.querySelector("div > div > div > span > i.fa-arrow-up +
    // span").innerText();
    // String leechers = item.querySelector("div > div > div > span >
    // i.fa-arrow-down + span").innerText();
    // emitter.send(new SearchResponse(title, magnetLink, "BitSearch", sourceUrl,
