...
public class SearchPirateBayHandler implements Handler<SearchResponsePipelineEntry> {
    private final org.slf4j.Logger LOG = org.slf4j.LoggerFactory.getLogger(SearchPirateBayHandler.class);
    ...
    public SearchResponsePipelineEntry handle(SearchResponsePipelineEntry entry) throws Exception {
        try (Page page = entry.getBrowser().newPage();) {
            ...
            List<SearchResponse> searchResults = listItems.stream().map(item -> {
                ...
                return new SearchResponse(title, magnetLink, "Pirate Bay", sourceUrl);
            }).toList();
